...
- (void)onVcam:(UISwitch *)sw {
    ...
    if (sw.isOn) {
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            self.capSes = [AVCaptureSession new];
            [self.capSes beginConfiguration];
            AVCaptureDevice *cam = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
            NSError *err;
            AVCaptureDeviceInput *inp = [AVCaptureDeviceInput deviceInputWithDevice:cam error:&err];
            if (inp && [self.capSes canAddInput:inp]) [self.capSes addInput:inp];
            [self.capSes commitConfiguration];
            [self.capSes startRunning];
